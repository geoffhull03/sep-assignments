...
def distancer(loc1, loc2)
  #A^2 + B^2 = C^2 ; need to find hypotenuse 
...
def nearest_neighbor(graph_of_cities, current_city)
  ...
  until graph_of_cities[0].nil?
    ...
    graph_of_cities.each do |choice|
      if distancer(current_city, next_city) > distancer(current_city, choice)
        next_city = choice
      end
    end
